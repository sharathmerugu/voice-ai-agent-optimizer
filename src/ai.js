...
const ANALYZE_SCHEMA = {
  ...
  required: ["callOutcomes", "issuePatterns", "testCases"],
  properties: {
    callOutcomes: {
      type: "array",
      items: {
    ...
    issuePatterns: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["type", "title", "severity", "frequency", "evidence"],
        properties: {
...
2. Identify the recurring issues. Use only these types: ${ISSUE_TYPES.join(", ")}.
   For each, give a specific title (not the type name), a severity, how many calls it
   appears in, and verbatim quotes as evidence.

3. Generate exactly 6 test cases for regression-testing this agent. Derive them from the
  ...
  return complete({ system: ANALYST_SYSTEM, prompt, schema: ANALYZE_SCHEMA });
}
...
the before/after diff, so the text must be ready to paste into the agent's configuration.`
  ...
  return complete({ system: EVALUATOR_SYSTEM, prompt, schema });
}
